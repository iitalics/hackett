...
Defines the @racket[name-id] as both a value and a pattern, such that
@racket[(name arg ...)] is equivalent to the @racket[body-pattern/expr]
with each parameter subsituted with the corresponding argument.

The @racket[body-pattern/expr] will be interpreted in two different ways:
as a pattern and as an expression. The pattern
@racket[(name-id arg-id ...)] will consistently match the value produced by
@racket[(name-id arg-expr ...)], @italic{if} every pattern used in
@racket[body-pattern/expr] also has this property.

@(hackett-examples
  (code:comment "Exp* is like an \"Exp/recur\"")
  (data (Exp* e)
    (Var* t:String)
    (App* e e)
    (Lam* t:String e)
